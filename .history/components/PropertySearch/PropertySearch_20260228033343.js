...
export const PropertySearch = () => {
  ...
  const handlePageClick = async (pageNumber) => {
    ...
    await router.push(`${router.query.slug.join("/")}?${qs}`);
  };
  ...
  // Загружаем посты при первом открытии страницы (без параметров в URL)
  useEffect(() => {
    if (!router.isReady) return;
    // Если URL не содержит параметров, загружаем все посты
